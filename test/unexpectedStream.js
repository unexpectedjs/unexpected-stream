...
describe('unexpected-stream', function () {
    ...
    describe('to yield output satisfying', function () {
        it('should buffer up the output of a readable stream that outputs buffers', function () {
            return expect(fs.createReadStream(fooTxtPath), 'to yield output satisfying', Buffer.from('foobarquux\n', 'utf-8'));
        });
        ...
        it('should convert to Buffer when a readable stream outputs both Buffer and string chunks', function () {
            ...
            return expect(stream, 'to yield output satisfying', Buffer.from('foobar'));
        });
    });
    ...
    describe('when piped through assertion', function () {
        it('should pipe the data through the proxy stream', function () {
            return expect(
                ...
                Buffer.from([0x1F, 0x8B, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x4B, 0xCB, 0xCF, 0x4F, 0x4A, 0x2C, 0x2A, 0x2C, 0x2D, 0xAD, 0xE0, 0x02, 0x00, 0xC8, 0x99, 0x6F, 0x44, 0x0B, 0x00, 0x00, 0x00])
            );
        });
        ...
        describe('with an array of Buffer instances', function () {
            it('should succeed', function () {
                return expect(
                    [ Buffer.from('foo', 'utf-8'), Buffer.from('bar', 'utf-8'), Buffer.from('quux\n', 'utf-8') ],
                    'when piped through',
                    zlib.createGzip(),
                    'to yield output satisfying',
                    Buffer.from([0x1F, 0x8B, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x4B, 0xCB, 0xCF, 0x4F, 0x4A, 0x2C, 0x2A, 0x2C, 0x2D, 0xAD, 0xE0, 0x02, 0x00, 0xC8, 0x99, 0x6F, 0x44, 0x0B, 0x00, 0x00, 0x00])
