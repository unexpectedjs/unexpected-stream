...
describe('unexpected-stream', () => {
  ...
  describe('to yield output satisfying', () => {
    it('should buffer up the output of a readable stream that outputs buffers', () => expect(
      fs.createReadStream(fooTxtPath),
      'to yield output satisfying',
      Buffer.from('foobarquux\n', 'utf-8')
    ));

    it('should buffer up the output of a readable stream that outputs strings', () => expect(
  ...
  describe('when piped through assertion', () => {
    it('should pipe the data through the proxy stream', () => expect(
      fs.createReadStream(fooTxtPath),
      'when piped through',
      zlib.createGzip(),
      'to yield output satisfying',
    ...
    it('should pipe the data through multiple proxy streams', () => expect(
      fs.createReadStream(fooTxtPath),
      'when piped through',
    ...
    it('should provide the target stream as the fulfillment value', () => expect(
      fs.createReadStream(fooTxtPath),
      'piped through',
      zlib.createGzip()
    ).then(targetStream => {
